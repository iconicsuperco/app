...
import { recentlyAddedTracksQuery } from '@/library/queries'
...
export function RecentlyAddedPage() {
  const tracks = useLiveQuery(() => recentlyAddedTracksQuery(), [])

  const trackIds = useMemo(() => (tracks ?? []).map((t) => t.id), [tracks])

  if (!tracks) {
    return <RecentlyAddedLoading />
  }

  if (tracks.length === 0) {
    return <RecentlyAddedEmpty />
  }
